...
//	Version: 1.9.7.1.uxm20200222
//	Summary: PHP-based admin tool to manage SQLite2 and SQLite3 databases on the web
//
//	Last maintenance updated: 2019-01-13
//	Maintainers:
//	   unixman (iradu@unix-world.org)
//	Copyright (C) 2017-2019, unix-world.org (XHTML to HTML5 + fixes)
//
...
$custom_functions = array(
	'md5', 'sha1', 'time', 'strtotime', 'base64_decode', 'base64_encode',
...
$allowed_extensions = array('sqlite','sqlite3');
...
$lang = array(
	...
	"confirm" => "Confirm",
	"cancel" => "Cancel",
	"save_as" => "Export As (.sql Extension will be added)", // "Save As",
	"options" => "Options",
	"no_opt" => "No options",
	"help" => "Help",
	"installed" => "installed",
...
define("VERSION", "1.9.7.1.uxm20200222");
define("PAGE", basename(__FILE__));
define("FORCETYPE", $force_extension); //force the extension that will be used (set to false in almost all circumstances except debugging)
define("SYSTEMPASSWORD", (string)$password); // Makes things easier.
...
if ($auth->isAuthorized())
{
	...
	if(isset($_POST['new_dbname']))
	{
		...
		elseif(preg_match('/^[_a-zA-Z0-9\-\.@#]+$/', (string)$_POST['new_dbname'])) {
			$str = (string) $_POST['new_dbname'];
			$dbname = $str;
			$dbpath = $str;
...
//if(is_file('themes/'.$theme)) $theme = 'themes/'.$theme; // allow themes to be dropped in subfolder "themes"
//if (file_exists($theme))
//	echo "<link href='{$theme}' rel='stylesheet' type='text/css'>", PHP_EOL; // an external stylesheet exists - import it
//else
...
*/
	...
	public function showError()
	{
		...
		$classSQLiteDatabase = false; // class_exists("SQLiteDatabase"); // fix by unixman
	...
	//	echo "<b>SQLiteDatabase</b>: ".($classSQLiteDatabase ? $lang['installed'] : $lang['not_installed'])."<br>"; // fix by unixman
		echo "<br>...".$lang['done'].".</i><br><br>";
		if(!$classPDO && !$classSQLite3) // && !$classSQLiteDatabase) // fix by unixman
